...
const searchClients = asyncHandler(async (req, res) => {
    ...
    const { q, leadStatus, assignedTo, sortBy, sortOrder } = req.query
    ...
    if (leadStatus && ["New", "In Progress", "Converted", "Lost"].includes(leadStatus)) {
...
// ─── Get Deleted Clients (Admin - Trash) ───────────
const getDeletedClients = asyncHandler(async (req, res) => {
    ...
    const [clients, totalClients] = await Promise.all([
        Client.find({ isDeleted: true })
            ...
            .populate("assignedTo", "fullname email role avatar")
            .populate("createdBy", "fullname email role avatar"),
        Client.countDocuments({ isDeleted: true })
    ])
    ...
    return res
        ...
        .json(new ApiResponse(200, {
            ...
            totalPages: Math.ceil(totalClients / limit),
            clients
        }, "Deleted clients fetched successfully"))
})
